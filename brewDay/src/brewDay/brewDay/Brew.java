...
public class Brew {
	private static float batchSize;
	private String date;
	...
	public void implement(Recipe recipe) throws SQLException { // implement the recipe that use select
		...
		while (getRI.next()) { // get amount from the class RecipeIngredient
			...
			ResultSet getAmountOfIngredient = Database
					.Select("SELECT Name, Amount FROM StorageIngredient WHERE Name = '" + nameOfRI + "'");
			while (getAmountOfIngredient.next()) {// get amount from the class Ingredient
				...
				if (nameOfIngredient.equals(nameOfRI)) { // for the same ingredient in RI and ingredient
					...
					if (amountOfIngredient >= amountOfRI) { // if the amount is enough for this operation, do other
															// matching
						continue;
		...
		if (flag == 0 && numberOfRI == numberOfIngredient) {// implement process
			...
			while (getRI1.next()) {// get amount from the class RecipeIngredient
				...
				ResultSet getAmountOfIngredient = Database
						.Select("SELECT Name, Amount FROM StorageIngredient WHERE Name = '" + nameOfRI1 + "'");
				while (getAmountOfIngredient.next()) {// get amount from the class Ingredient
					float amountOfIngredient = getAmountOfIngredient.getFloat("Amount");
					float result = (float) amountOfIngredient - (float) amountOfRI1; // subtract and update the amount
																						// in class Ingredient
			...
			String sqlBrew = "Insert Into Brew Values (NULL, " + batchSize + " , '" + date + "' , " + Rid + ")"; // insert
																													// brew
																													// data
																													// into
																													// database
	...
	public static boolean enoughIngredient(Recipe recipe) throws SQLException {
		int flag = 0; // a mark
		int Rid; // recipe ID, for searching use
		int numberOfRI = 0;
		int numberOfIngredient = 0;
	...
	public static boolean recommend(float batchsize) throws SQLException {
		...
		while (getRecipe.next()) {
			...
			// get the number of line
			ResultSet getLine = Database
					.Select("SELECT COUNT(*) as count FROM RecipeIngredient Where RecipeID=" + getRID);
			while (getLine.next()) {
			...
			if (getA < tempgetAmount) {
				...
				l.add("Recipe <" + getName + "> Storage of " + getIngredientName + " are not enough"
						+ ", you should buy " + (-need));
				// System.out.println("For recipe <"+getName+"> Storage of "+getIngredientName
				// +" not enough"+", you should buy "+ (-need));
			} else {
				arr[getRID]++;
